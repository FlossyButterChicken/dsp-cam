...
while True:
	...
	# gray scale for masking
	result_gray = cv2.cvtColor(result, cv2.COLOR_BGR2GRAY)

    # generate mask (black areas to white, non-black areas to black) then dilate
	mask = cv2.inRange(result_gray, 0, 1)    
	mask = cv2.dilate(mask, np.ones((5, 5)))

	# save mask
	#cv2.imwrite('masks.jpg', mask)      7
                
	# paint/fill
	result = cv2.inpaint(result, mask, 3, cv2.INPAINT_TELEA)
	
	# color change
	#stitched_frame = cv2.cvtColor(result, cv2.COLOR_BGR2RGB)
